...
from CDFSanya.items import CdfProductItem, CdfProductItemLoader
import requests
...
class CdfsanyaSpider(scrapy.Spider):
    ...
    def parse_two(self, response):
        ...
        for item in items:
            ...
            self.headers_three["Referer"] = "http://www.cdfgsanya.com/product.html?productId={}&goodsId={}&warehouseId={}&brandId={}".format(productId,goodsId,warehouseId,brandId)
            ...
            clasi_response = requests.get(clasification_api, headers=self.headers_for_get).text


            product_ids = {}
            product_ids["productid"] = productId
            product_ids["goodsid"] = goodsId
            product_ids["styleid"] = styleId
            product_ids["warehouseid"] = warehouseId
            product_ids["product_url"] = self.headers_three["Referer"]
            product_ids["clasi_response"] = clasi_response


            yield Request(detail_page_api, headers=self.headers_three, meta=product_ids, callback=self.parse_three, dont_filter=True)
